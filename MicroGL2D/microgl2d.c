...
static inline int MGL_sin(int angle)
{
...
//angle - угол в градусах
static inline int MGL_cos(int angle)
{
...
//Вычисляет результирующий цвет в точке при наложении объектов с учетом прозрачности
static inline uint16_t MGL_NewColor_rgb(uint16_t col_f, uint8_t r, uint8_t g, uint8_t b, uint16_t a)
{
	if (a) {
		a++;
#ifdef MGL_SWAP_BYTES
						   	   ...
						   	   uint32_t color)
{
	...
	if (obj->texture) {
		...
		for (x = x_start; x <= x_end; x++) {
			...
			buffer[x - x0] = MGL_NewColor_rgb(buffer[x - x0], r, g, b, tr);
		}
	}
