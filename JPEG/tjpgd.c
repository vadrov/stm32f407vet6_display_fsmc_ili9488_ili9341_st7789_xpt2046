...
extern uint8_t JPEG_color_mode;
...
*/
	yy <<= 6;
	asm volatile (	"movs 	r0, #113							\n\t"
				    "mla	r0, r0, %[cb], %[yy]				\n\t"
					"usat	%[color], #5, r0, asr #9			\n\t" //b
					"asr 	%[yy], %[yy], #1					\n\t"
					"movs 	r0, #45								\n\t"
					"mla	r0, r0, %[cr], %[yy]				\n\t"
					"usat	r0, #5, r0, asr #8					\n\t" //r
					...
					"mls	r0, r0, %[cb], %[yy]				\n\t"
					"usat	r0, #5, r0, asr #8					\n\t" //g
					"orr 	%[color], %[color], r0, lsl #6		\n\t"
					: [color]"=&r"(color), [yy]"+r"(yy)
					: [cr]"r"(cr), [cb]"r"(cb)
					: "r0");
	return color;
}
...
)
{
	...
	while (ndata) {	/* Process all tables in the segment */
		...
		{	/* Create fast huffman decode table */
			unsigned int td, ti;
#if (JD_FAST_OPTIMIZE == 0) || defined(JD_MEM_NO_OPTIMIZE)
			unsigned int span;
#endif
			uint16_t *tbl_ac = 0;
			uint8_t *tbl_dc = 0;
...
#else
	int32_t *tmp = (int32_t*)(jd->workbuf + jd->offset_load);
#endif
	//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	int d, e;
...
)
{
	...
	if (!JD_USE_SCALE || jd->scale != 3) {	//Не для масштабирования 1/8
		if (jd->color_format < 2) { //jd->color_format = 0 - r8g8b8, jd->color_format = 1 - r5g6b5
			for (iy = 0; iy < my; iy++) {
				pc = py = jd->mcubuf;
		...
		if (JD_USE_SCALE && jd->scale) {
			...
			int k_byte = 1;
			if (!jd->color_format) { //RGB888 (24-bit/pix)
				k_byte = 3;
			}
			else if (jd->color_format & 1) { //jd->color_format = 1, jd->color_format = 3
				k_byte = 2;
			}
			a *= k_byte;
			for (iy = 0; iy < my; iy += w) {
	...
	if (rx < mx) {	// если rx < mx, то перестраиваем пиксели под область вывода
		uint8_t *s, *d;
		s = d = (uint8_t*)workbuf;
		if (jd->color_format & 1) {	//jd->color_format = 1 - R5G6B5 (16-bit/pix), jd->color_format = 3 - Grayscale_16 (16-bit/pix)
			unsigned int rx1 = rx;
			...
			if (!jd->color_format) { //R8G8B8 - 3 байта на пиксель
				rx *= 3;
				mx *= 3;
			}
			for (y = 1; y < ry; y++) { //Перемещаем пиксели в массиве для новой области вывода, начиная со 2 строки
...
)
{
	...
	void *workbuf = jd->workbuf + jd->offset;
#else
	void *workbuf = jd->workbuf + jd->offset_out;
#endif
	//+++++++++++++++++++++++++++++++++++++++++++++++++++++
	unsigned int ix, iy, mx, my, rx, ry;
	int yy, cb, cr;
	...
	if (rx < mx) {	// если rx < mx, то перестраиваем пиксели под область вывода
		uint8_t *s, *d;
		s = d = (uint8_t*)workbuf;
		if (jd->color_format & 1) {	//R5G6B5 (16-bit/pix), Grayscale_16 (16-bit/pix)
			unsigned int rx1 = rx;
			rx *= 2; //2 байта на пиксель
			mx *= 2;
			for (y = 1; y < ry; y++) { //Перемещаем пиксели в массиве для новой области вывода, начиная со 2 строки
...
)
{
...
#endif
	/****************************** +++++++++++++++++++++++++++ ****************************/
	jd->color_format = JPEG_color_mode; //записываем в переменную значение формата цвета
	/***************************************************************************************/
	...
	for (;;) {				/* Parse JPEG segments */
		...
		case 0xDA:	/* SOS - Start of Scan */
			...
			jd->offset_value = len;
			jd->offset = jd->offset_load = jd->offset_out = 0;
			//----------------------------------------------------------------------
...
mcu_parameters mcu_out_par = {0}, mcu_load_par = {0};
SemaphoreHandle_t mcuSemaphore;

static void mcu_task_load(void *param)
{
	mcu_parameters* par = (mcu_parameters*)param;
	par->rc = mcu_load(par->jd);
	xSemaphoreGive(mcuSemaphore);
	vTaskDelete(NULL);
}

static void mcu_task_out(void *param)
{
	mcu_parameters *par = (mcu_parameters*)param;
	par->rc = mcu_output(par->jd, par->outfunc, par->x, par->y);
	xSemaphoreGive(mcuSemaphore);
	vTaskDelete(NULL);
}
#endif
...
)
{
...
#ifdef ESP_TWO_CORE
	unsigned int x_prev, y_prev;
	mcuSemaphore = xSemaphoreCreateCounting(2, 0);
	int f_load_first = 0;
#endif
	...
	for (y = 0; y < jd->height; y += my) {		/* Vertical loop of MCUs */
		for (x = 0; x < jd->width; x += mx) {	/* Horizontal loop of MCUs */
			...
			mcu_load_par.jd = jd;
			mcu_out_par.jd = jd;
			mcu_out_par.x = x_prev;
			mcu_out_par.y = y_prev;
			mcu_out_par.outfunc = outfunc;
			xTaskCreatePinnedToCore(mcu_task_load, "mcu_core0", 2000, (void*)&mcu_load_par, 4, NULL, 0);
			xTaskCreatePinnedToCore(mcu_task_out, "mcu_core1", 2000, (void*)&mcu_out_par, 4, NULL, 1);
			xSemaphoreTake(mcuSemaphore, portMAX_DELAY);
			xSemaphoreTake(mcuSemaphore, portMAX_DELAY);
			if (mcu_load_par.rc != JDR_OK) return mcu_load_par.rc;
			if (mcu_out_par.rc != JDR_OK) return mcu_out_par.rc;
			jd->offset_load = jd->offset_value - jd->offset_load;
			jd->offset_out = jd->offset_out - jd->offset_out;
			x_prev = x;
			y_prev = y;
#else
			rc = mcu_load(jd);					/* Load an MCU (decompress huffman coded stream, dequantize and apply IDCT) */
...
#ifdef ESP_TWO_CORE
	if (f_load_first) {
		rc = mcu_output(jd, outfunc, x_prev, y_prev);
	}
#endif
	return rc;
}
